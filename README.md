...
Trên `n600` (chạy 24/09/2026, `results/ladder5_steps_n600.csv`), bị gán prior sai **không** tốn thêm gì ngoài việc mất prior đúng, với cận tương đương thu từ 8,5 xuống 4,49 điểm. Nhưng câu cũ "khi prior đã mất thì từ có thật hay không không còn quan trọng" **không đứng**: từ thật mà không liên quan làm model trả lời **kém hơn** ký hiệu trơn 3,76 điểm, và bậc này nằm trọn trong một đợt chạy. Bậc đầu tiên vắt qua hai đợt chạy (`KEEP` chạy khoảng 16/09); `scripts/check_drift.py` đã gửi lại các prompt của đợt cũ (24/09/2026, 5,54 USD) và **không thấy độ trôi**: độ chính xác mới trừ cũ là -0,38 [-1,33 ; +0,55] trên 5.256 ô, và không điều kiện hay model nào tách khỏi 0 (`results/drift_check.csv`), nên nó đứng.
