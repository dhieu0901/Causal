...
prompts (GPT-4.1: its three models, averaged per item, as in every B5 table).
...
def draws(prefix):
    ...
    `prefix`'s records: analyze_answer_change.harm, pointed at other files.
    The draws are the confirmatory sample's (same items, seed and prompts), so
    their groups do not depend on which model answered."""
    G = pd.read_csv(RESULTS / "answer_change.csv")
    G = G[G["sample"].eq("conf") & G.lexicon.eq("KEEP")][["item", "cond", "group", "route",
                                                          "implied_answer"]]
    AC.SAMPLES["b7_" + prefix] = (AC.SAMPLES["conf"][0], [f"pilot_raw_{prefix}{{}}.csv"])
    h = pd.concat([AC.harm("b7_" + prefix, lex, "ORACLE") for lex in ("KEEP", "PSEUDO")])
...
def main() -> int:
    ...
    print("=" * 92)
    rows = {f: vs_raw(data[f], f) + ladder(data[f], f, FAMILIES[f][0]) for f in FAMS}
    ...
    X = {f: draws(FAMILIES[f][1]) for f in FAMS}
    ...
    A = pd.DataFrame(accuracy(data["gpt"], "gpt") + accuracy(data["luna"], "luna"))
    ...
    for part in R.part.unique():
        ...
        for r in R[R.part == part].itertuples():
            print(f"    {r.family:5s} {r.lexicon:22s} {r.quantity:60s} {r.estimate_pp:+7.2f} "
